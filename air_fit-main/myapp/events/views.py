...
def SignupPage(request):
    if request.method == 'POST':
        ...
        if pass1 != pass2:
            ...
            message += "Click here to approve: {request.build_absolute_uri(reverse('approve_user_creation', kwargs={'token': token}))}"
...
def approve_user_creation(request, token):
    ...
    if request.method == 'POST':
        if 'approve' in request.POST:
            ...
            message += "Click here to login: {request.build_absolute_uri(reverse('login'))}"
            from_email = 'access.users0@gmail.com'
